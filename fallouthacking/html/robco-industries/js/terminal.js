...
window.addEventListener('message', function(event) {
	switch(event.data.action) {
		case "open": {
      Difficulty = event.data.length;
      AttemptsRemaining = event.data.retries;
      MaxAttempts = event.data.retries;
			$("#minigame").show();
			break;
		}
		case "close": {
...
GetWords = function(length, count) {
	...
	AllWords.forEach((word) => {
		if(word.length == length  && WordCount <= count) {
...
FillWordColumns = function()
{
	...
	i = 0;
	while (i < (Words.length-1) / 2)
	{
		var pos = start + i * Math.floor(AllChars.length / (Words.length / 2));
		for (var s = 0; s < Difficulty; s++)
		{
			var word = Words[i + (Words.length-1) / 2].toUpperCase();
			$(AllChars[pos + s]).addClass("word").text(word[s]).attr("data-word", word);
		}
		i++;
	}
